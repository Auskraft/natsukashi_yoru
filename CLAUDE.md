...
- ✅ **Опубликовано в RuStore** (v1.0.0 прошла модерацию). `STORE_DESCRIPTION.md`
...
- ✅ **Модуль «Оцените приложение»** (`core/rating/`, `url_launcher`): рейтинг-гейт
  ...
  «📄 Документы» — последний.
- 🔢 **Версия `1.0.1+2`** (готовится обновление; v1.0.0 уже в сторе). При каждом
  апдейте поднимать versionCode (число после `+` в `pubspec.yaml`), затем пересобрать.
